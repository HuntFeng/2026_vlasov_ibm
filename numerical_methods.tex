...
\begin{equation}
	F_{i+1/2} = \begin{cases}
		\sum_{k=s+1}^i f_k^n + \varphi\left( f_{s}^n + \frac{1}{6}(2 - \abs{\varphi})(1-\abs{\varphi})(f_{s+1}^n - f_{s}^n) + \frac{1}{6}(1 - \abs{\varphi})(1+\abs{\varphi})(f_{s}^n - f_{s-1}^n)\right),      & \varphi > 0, \\
		-\sum_{k=i+1}^{s-1} f_k^n + \varphi\left( f_{s}^n + \frac{1}{6}(2 - \abs{\varphi})(1-\abs{\varphi})(f_{s}^n - f_{s-1}^n) + \frac{1}{6}(1 - \abs{\varphi})(1+\abs{\varphi})(f_{s+1}^n - f_{s}^n)\right), & \varphi < 0.
	\end{cases}
	\label{eq:flux_3rd_order}
\end{equation}
where $\varphi = v\Delta t/\Delta x - [v\Delta t/\Delta x]$ is related to CFL number, and $s = i - \lfloor\varphi\rfloor$.

To preserve positivity of the distribution function, a modified flux based on \cite{xiong2014high} is used.
\begin{equation}
	\hat{F}_{i+1/2} = L_{i+1/2} \left( F_{i+1/2} - f_{i+1/2} \right) + f_{i+1/2}.
	\label{eq:modified_flux}
\end{equation}
The parameter $L_{i+1/2}$ is designed to ensure the positivity of the distribution function at the next time step, $f_i^{n+1}$, in Eq.~(\ref{eq:distribution_update}). The $f_{i+1/2} = \max(\varphi f_i, 0)$ is the boundary-aware first order upwind flux.
...
To better describe the modified flux, let $d_{i\pm1/2} = F_{i\pm1/2} - f_{i\pm1/2}$, $\delta_i^n = 0 - (f_i^n + f_{i-1/2} - f_{i+1/2})$ and $p_i = d_{i-1/2} - d_{i+1/2} - \delta_i^n$. The parameter $L_{i+1/2} = \min(\Lambda_i^r, \Lambda_{i+1}^l)$, where $\Lambda_i^r, \Lambda_{i+1}^l$ are defined as follows,
